...
class CalendarTransaction extends DatabaseManager{
  ...
  public function getAllTransactionRecord(){
		  session_start();
        	$stmt=$this->connect()->prepare('SELECT DISTINCT q.transaction_id as id,r.name as type,fc.name as title,fc.photo as photo,q.date_filling as startDate,q.date_return as endDate FROM reservation_facilities q ,reservation_type r, users s,reservation_motorpool m,facilities_type fc, reservation_status st WHERE q.reservation_type_id = r.id AND q.user_id = m.user_id AND q.user_id = s.id AND q.facilities_type_id = fc.id AND q.status_id = st.id AND st.id =1 UNION ALL SELECT DISTINCT m.transaction_id as id,r.name as type,tp.name as title,tp.photo as photo ,m.date_filling as startDate,m.date_return as endDate FROM reservation_motorpool m, reservation_type r, users s,reservation_facilities q,motorpool_type tp, reservation_status ts WHERE m.reservation_type_id = r.id AND m.user_id = s.id AND m.motorpool_type_id = tp.id AND m.status_id = ts.id AND ts.id = 1');
		    	$stmt->execute();
